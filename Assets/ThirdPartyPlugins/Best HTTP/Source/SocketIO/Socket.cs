...
namespace BestHTTP.SocketIO
{
    ...
    public sealed class Socket : ISocket
    {
        ...
        /// <summary>
        /// Custom created method for sending args type as JSONNode.
        /// </summary>
        /// <param name="eventName">Name of event to Emitted</param>
        /// <param name="callback">Register a callback, if any</param>
        /// <param name="args">Already created JSONNode to send on server side</param>
        /// <returns>Socket</returns>
        public Socket EmitJSONData(string eventName, SocketIOAckCallback callback, JSONNode args)
        {
            bool blackListed = EventNames.IsBlacklisted(eventName);
            if (blackListed)
                throw new ArgumentException("Blacklisted event: " + eventName);
            ...
            // For converting the given JSONNode to JSONArray for 
            // converting parsable string on server side
            JSONNode data = new SimpleJSON.JSONArray();
            data.Add(eventName);           
            data.Add(args);
